import { revalidatePath } from "next/cache"
import { NextRequest, type NextResponse } from "next/server"

const BearerToken = "eyJhbGciOiJSUzI1NiIsImtpZCI6IjlTZWo3eXJYdGo1TzNDVV9JeG5EU2ciLCJ0eXAiOiJhdCtqd3QifQ..tLa36ORl8P-gbuWNdSr9e1spdaehVaDELSzmTAX5A-Zk0_BiypBm65PK-0dYGKLriHpZhKHm14Y7zbJXE6YsWzB-pn1lTjfExWgVe0BoF2Nl65DLdgoobk7RKmwY_70kBaqBiJ8xkCNl_T9Ec10WsbcjsRDJr6ICALPcKHggYCBz1tbWaqJjlClPWFFqwJ-N6eqaGiBmcQ5Yz5tMXYDWeaW3gWnmRkl9ezc4vZoa4fkYts1ZxLWbebNfv_R7ViIp-MvRUGfgsJpksmlb1pFZjO5YeGlnFS_j3NeG8u6viIk4jaRqABZ3vdxfC__JjbFH5FSZB34MWaG-eXz2Uvo2qg";

async function GetHandler(req: any, res: NextResponse) {
  revalidatePath("/api/menus");

  try {
    const response = await fetch('https://cloud.squidex.io/api/content/breakingmv/articles/query', {
      headers: {
        Authorization: `Bearer ${BearerToken}`,
        "Content-Type": "application/json"
      },
      method: "POST",
      body: JSON.stringify({
        "q": {
            "filter": {
                "and": [
                    {
                        "path": "data.latestNews.iv",
                        "op": "eq",
                        "value": true
                    },
                    // {
                    //     "path": "data.title.iv",
                    //     "op": "contains",
                    //     "value": "ކޮމިޓީ"
                    // }
                ]
            },
            "take": 1,
            "sort": [
                {
                    "path": "lastModified",
                    "order": "descending"
                }
            ]
        }
      })
    });

    const data = await response.json();
    const articleId = data.items[0].id;

    const response2 = await fetch(`https://cloud.squidex.io/api/content/breakingmv/articles/${articleId}`, {
      headers: {
        Authorization: `Bearer ${BearerToken}`,
      },
    });

    const articleLatest = await response2.json();

    return new Response(JSON.stringify(articleLatest), { status: 200 })
  } catch (error) {
    // res.status(500).json({ error: 'Internal server error' });

    return new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 })
  }
}

export { GetHandler as GET };